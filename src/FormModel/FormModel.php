...
class FormModel
{
    ...
    public function modelForm($model, $fillable, $location, $relations, $method = 'GET')
    {
        ...
        $return .= '<form class="form-horizontal" action="' . $location . '" method="' . $real_method . '" enctype="multipart/form-data" @submit.prevent="makeRequest">';
    ...
    private function modelInput(Model $model, $input, $old_input = null, $edit = false)
    {
        ...
        if (config('kregel.formmodel.using.bootstrap')) {
            if ($type === 'select') {
                $return .= $this->bootstrapBoolSelect([
                    'type' => $type,
                    'class' => 'form-control',
                    'name' => $old_input,
    ...
    public function boolSelect($options = [])
    {
        ...
        return $label . '<select' . $this->attributes($options) . '><option value="0">No</option><option value="1">Yes</option></select>' . !empty($label) ? '</label>' : '';
    }
